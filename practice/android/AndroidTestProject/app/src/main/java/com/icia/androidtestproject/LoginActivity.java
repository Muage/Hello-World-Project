...
//                        System.out.println("/////" + response.body());
                        LoginSession.setUserName(LoginActivity.this, response.body().getU_name());  // 유저 이름 세션에 저장
                        LoginSession.setU_code(LoginActivity.this, response.body().getU_code()); // 유저 코드 세션에 저장
                        if(LoginSession.getUserName(LoginActivity.this).length() != 0) {    // 세션에 저장된 유저가 있으면
                            Intent intent = new Intent(LoginActivity.this, MainActivity.class); // 로그인 성공 후 액티비티 이동
                    ...
                    public void onFailure(Call<UserVO> call, Throwable t) {
                        ...
                        System.out.println("..........ERR :: LoginActivity - SIGN IN 클릭 : " + t.toString());
                    }
                });
            }
        });

        // create an account 클릭 시
    ...
    private void updateKakaoLoginUI() {
        UserApiClient.getInstance().me(new Function2<User, Throwable, Unit>() {
            ...
            public Unit invoke(User user, Throwable throwable) {
                // 로그인 되어 있을 시
                if(user != null) {
                    Log.d(TAG, "LoginActivity - updateKakaoLoginUI :: id: " + user.getId());    // u_code 생성에 사용
