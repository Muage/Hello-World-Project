...
router.get('/list.json', function(req, res) {
    var sql = 'select *, date_format(hiredate, "%Y년 %m월 %d일") fhire, format(salary, 0) fsalary from professors';
...
router.get('/insert', function(req, res) {
  ...
  db.get().query(sql, function(err, rows) {
    res.render('index', { title: '교수등록', pageName: 'pro/insert.ejs', ncode: rows[0].ncode })
  })
})
...
router.post('/insert', function(req, res) {
  ...
  // console.log(`${pcode}\n${pname}\n${dept}\n${hiredate}\n${title}\n${salary}`)
  var sql = 'insert into professors(pcode, pname, dept, hiredate, title, salary) values(?, ?, ?, ?, ?, ?)';
...
router.get('/update', function(req, res) {
  ...
  var sql = 'select *, date_format(hiredate, "%Y") yy, date_format(hiredate, "%m") mm, date_format(hiredate, "%d") dd from professors where pcode = ?';
  db.get().query(sql, [pcode], function(err, rows) {
    res.render('index', { title: '교수정보수정', pageName: 'pro/update.ejs', row:rows[0]})
  })
})
...
router.post('/update', function(req, res) {
  ...
  db.get().query(sql, [pname, dept, title, salary, hiredate, pcode], function(err, rows) {
    res.redirect('/pro');
  })
})
