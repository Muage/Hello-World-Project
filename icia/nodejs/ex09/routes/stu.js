...
router.get('/list.json', function(req, res) {
  ...
  db.get().query(sql, function(err, rows) {
    count = rows[0].cnt;
    var sql = 'select *, date_format(birthday, "%Y-%m-%d") fbirthday from view_stu order by dept limit ?, 5';
...
router.get('/update', function(req, res) {
  ...
  var sql = 'select *, date_format(birthday, "%Y년 %m월 %d일") fbirthday from view_stu where scode = ?';
  db.get().query(sql, [scode], function(err, rows) {
    res.render('index', { title: '학생정보', pageName: 'stu/update.ejs', row: rows[0] });
  })
})
