...
select c.*, pname, p.dept
from courses c, professors p
where instructor = pcode;

select * from view_cou;

select * from enrollments where lcode = 'C301';

create view enroll_stu as
select e.*, sname, dept, year
from enrollments e, students s
where e.scode = s.scode;

select * from enroll_stu where lcode = 'C301';

select * from enrollments where scode = '92414029';

create view enroll_cou as
select e.*, lname, hours, room
from enrollments e, courses c
where e.lcode = c.lcode;

select * from enroll_cou where scode = '92414029';

select max(pcode)+1 ncode from professors;

select count(*) cnt from students where advisor='510';
select count(*) cnt from courses where instructor='510';
...
update enrollments set grade = 95
where scode = '92414029' and lcode = 'C301';

select * from view_stu where scode = '92414029';

select * from enroll_cou where scode = '92414029';

desc enrollments;

alter table enrollments modify edate datetime default now();